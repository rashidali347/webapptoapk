...
#   Important for usage:
#
#   ls -l gradlew # check execution permissions
#   chmod +x gradlew # set execution permissions if necessary
#
##############################################################################
...
if [ -n "$JAVA_HOME" ] ; then
    if [ -x "$JAVA_HOME/bin/java" ] ; then
        # Others might emit warnings about deprecated JVM flags, but we can't control them.
...
if $cygwin ; then
    ...
    for arg in "$@" ; do
        ...
        if [ $CHECK -ne 0 ] && [ $CHECK2 -eq 0 ] ; then                    ### Added a condition
            eval `echo "arg$i"`=`cygpath --path --windows "$arg"`
        else
            eval `echo "arg$i"`="\"$arg\""
        fi
        i=`expr $i + 1`
